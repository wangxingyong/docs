        ...
        throws MojoExecutionException
    {
        ...
        try
        {
            ...
            if ( executedProject.getCollectedProjects().size() > 0 )
            {
                ...
                String projectPath =
                        new File( executedProject.getBasedir(), executedProject.getArtifactId() +"-"+executedProject.getVersion()+ ".iml" ).getAbsolutePath();
                ...
                for ( Iterator i = executedProject.getCollectedProjects().iterator(); i.hasNext(); )
                {
                    ...
                    String modulePath = new File( p.getBasedir(), p.getArtifactId() +"-"+p.getVersion()+ ".iml" ).getAbsolutePath();
                    m.addAttribute( "filepath",
                                    "$PROJECT_DIR$/" + toRelative( executedProject.getBasedir().getAbsolutePath(),
                                                                   modulePath ) );
                }
